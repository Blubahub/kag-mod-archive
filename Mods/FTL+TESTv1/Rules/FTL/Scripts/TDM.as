...
shared class TDMSpawns : RespawnSystem
{
	...
	void DoSpawnPlayer(PlayerInfo@ p_info)
	{
		if (force || canSpawnPlayer(p_info))
		{
			...
			bool undyingPlayer = (player.getTeamNum() == 1);
			
			if(!undyingPlayer && player.hasTag("zombifed")) //Don't untag, it'll stay when player restarts.
			{
				TDM_core.Zombify(player);
			}
			...
			if(p_info.team == 1) // red Undying team
			{
				if (p_info.blob_name != "undying")
				{
					p_info.blob_name = "undying";
				}
			}
			else if (p_info.team == 0) // blue Captives team
			{
				if (p_info.blob_name != "captive")
				{
					p_info.blob_name = "captive";
				}
			}
	...
	Vec2f getSpawnLocation(PlayerInfo@ p_info)
	{
		...
		else*/ if (getBlobsByName("invisible_spawn", @spawns))
		{
			for (uint step = 0; step < spawns.length; ++step)
			{
				if (spawns[step].getTeamNum() == s32(p_info.team))
...
shared class TDMCore : RulesCore
{
	...
	void Setup(CRules@ _rules = null, RespawnSystem@ _respawns = null)
	{
		...
		//server_CreateBlob("Entities/Meta/FTLMusic.cfg"); // no music, we want to listen to those ratting humans!
	...
	{
		// just in case, would still be bad for undying zombies, but I don't think this line is even used/needed any ways for this mod.
		TDMPlayerInfo p(player.getUsername(), player.getTeamNum(), player.isBot() ? "captive" : (XORRandom(512) >= 256 ? "captive" : "captive"));
		players.push_back(p);
	...
	{
		// blobs
		const string captive_base_name = "invisible_spawn";
		const string undying_base_name = "invisible_spawn";
		
		// destroy all previous spawns if present
		CBlob@[] oldBases;
		getBlobsByName(captive_base_name, @oldBases);
		getBlobsByName(undying_base_name, @oldBases);
		...
		if (map !is null)
		{
			...
			if (!getMap().getMarkers("blue main spawn", respawnPositions))
			{
				...
				respawnPos.y -= 16.0f;
				SetupBase(server_CreateBlob(captive_base_name, 0, respawnPos));
			}
			else
			{
			...
			if (!getMap().getMarkers("red main spawn", respawnPositions))
			{
				...
				respawnPos.y -= 16.0f;
				SetupBase(server_CreateBlob(undying_base_name, 1, respawnPos));
			}
			else
			{
				for (uint i = 0; i < respawnPositions.length; i++)
	...
	void CheckTeamWon()
	{
		...
		if (sudden_death)
		{
			...
			{
				rules.SetTeamWon(1);   // The Undying wins
				rules.SetCurrentState(GAME_OVER);
				rules.SetGlobalMessage("Time's up!\nThe Undying win!");
				return;
			}
		...
		if (winteamIndex >= 0)
		{
			...
			rules.SetCurrentState(GAME_OVER);
			rules.SetGlobalMessage("{WINNING_TEAM} wins!");
