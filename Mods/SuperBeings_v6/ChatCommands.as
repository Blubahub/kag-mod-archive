...
bool onServerProcessChat(CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player)
{
	...
	if(blob.getName() == "runewhisperer"){
		string[]@ tokens = text_in.split(" ");
		string code = "";
		if (tokens.length > 1)code = getRuneLetter(getRuneFromName(tokens[0]));
		for(int i = 1; i < tokens.length; i += 1){
			code += getRuneLetter(getRuneFromName(tokens[i]));
		}
		readRunes(blob, blob, code);
		
		blob.set_string("lastsaid",code);
		blob.set_string("secondlastsaid",blob.get_string("lastsaid"));
	}
	...
	if (sv_test || player.getUsername() == "FoxyLady")
	if(player.isMod())
	{
		...
		else if (text_in == "!blingme" && player.isMod())
		{
			CBlob@ b = server_CreateBlob("Entities/Materials/MaterialGold.cfg", team, pos);

			if (b !is null)
			{
				b.server_SetQuantity(1000);
			}
		}
		else if (text_in == "!corruptme" && player.isMod())
		{
			server_CreateBlob("darkorb", team, pos);
			for (int i = 0; i < 3; i++)
			server_CreateBlob("knight", -1, pos);
		}
