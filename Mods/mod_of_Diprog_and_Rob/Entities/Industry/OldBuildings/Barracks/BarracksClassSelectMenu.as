...
void addBarracksPlayerClass( CBlob@ this, string name, string iconName, string configFilename, string description)
{
    if (!this.exists("barracksplayerclasses"))
    {
        NewPlayerClass[] classes;
        ...
        for (uint i = 0 ; i < classes.length; i++)
        {
            ...
            if (pclass.item == "coins")
            {
                ...
                if (player !is null)
                {
                    ...
                    if (coins >= parseInt(pclass.itemsDesc))
                    {
                        hasItem = true;
                        description = pclass.name + "\nRequires " + pclass.itemsDesc + " coins";
                    }
                    else
                    {
                        description = pclass.name + "\nRequires " + pclass.itemsDesc + " coins";
