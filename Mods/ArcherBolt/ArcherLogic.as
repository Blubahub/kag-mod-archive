...
CBlob@ CreateArrow(CBlob@ this, Vec2f arrowPos, Vec2f arrowVel, u8 arrowType)
{
	...
	if (arrowType == ArrowType::normal || arrowType == ArrowType::bomb)
	{
		config = "ballista_bolt";
		arrowPos = Vec2f(arrowPos.x, arrowPos.y - 1);
		arrowVel = Vec2f(arrowVel.x, arrowVel.y);
	}
		
	CBlob@ arrow = server_CreateBlobNoInit(config);
	if (arrow !is null)
	{
		// fire arrow?
		arrow.set_u8("arrow type", arrowType);
		arrow.Init();
		if (arrowType == ArrowType::bomb) arrow.Tag("bomb ammo");
