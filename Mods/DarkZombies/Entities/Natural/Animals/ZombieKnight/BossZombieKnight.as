...
{
	if ((getLocalPlayer().getUsername() == "0DarkShadow0" || getLocalPlayer().getUsername() == "xTheSwiftOnex") && getRules().get_bool("target lines"))
	{
		CBlob@ blob = this.getBlob();
						...
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							if (other !is null)
							{
								...
								else if (!hit_block)
								{
									...
									if (other.getName() == "wooden_platform" || other.getName() == "GoldBrick" || other.getName() == "GoldDoor")
									{
    ...
    if (this.getHealth() <= gibHealth)
    {
		...
		if (getNet().isServer())
		{
			...
			if (r<10 && getRules().get_bool("scrolls_spawn"))
			{
				...
				if (r == 9)
					server_MakePredefinedScroll( hitterBlob.getPosition() + Vec2f(0,-3.0f), "fish_transform" );
			}
			else
			{
...
if (hitBlob !is null)
	{
		...
		s32 boss_zombie_knockback_power = getRules().get_s32("boss_zombie_knockback_power");
		//printf("FORCE ADDED");
		//printf("forcePow.x = "+forcePow.x+" forcePow.y = "+forcePow.y+" this.getMass() = "+this.getMass()+" damage = "+damage);
		Vec2f force = forcePow * this.getMass() * damage * boss_zombie_knockback_power;
